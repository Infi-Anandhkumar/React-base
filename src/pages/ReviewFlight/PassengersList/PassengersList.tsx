...
const PassengersList = (props: PassengerListProps) => {
  ...
  const [, SsetSsrPNRData] = sessionStorageAccessor<any>("ssrPNRData");
        ...
        seatRowItem.forEach((seatColItem: any) => {
          if (
            ...
            paxSSRDatatemp[count].rebookSsrData[props.tripIndex[0]].ssrData[props.tripIndex[1]].seatDetail.selected =
              setValue === "" ? false : true;
            setValue !== "" && (seatColItem.icon = "SelectedSeatIcon");
            count++;
          } else {
  ...
  const handleButtonClick = (_mealType: string, value: string, _set: boolean) => {
      ...
      <Col xs={24} xl={24}>
        {paxData.length ? (
          <>
            ...
            <Row>
              <Col xs={24} xl={24} className="mr-15 ml-15 cls-flightSeatList">
                {tabSelected === t("meals")
                  ? 
                    ...
                    if (typeof mealType === 'string') {
                      return (
                        <React.Fragment key={mealType}>
                          <Select
                            suffixIcon={
                              <Text className="Infi-04_DropdownArrow fs-8 p-clr cls-dropdown-icon" />
                            }
                            ...
                            dropdownRender={() => (
                              <div className="cls-meals-dropdown">
                                {allSelectData[mealType].length ? allSelectData[mealType].map((value:any, mealIndex:number) => (
                                  <div
                                    className="cls-options d-flex space-between align-center px-10 py-5"
                                    key={mealIndex}
                                  >
                          ...
                          >
                            <Select.Option value={mealType.replace("_", " ")}>
                              <Text className="fs-18 Infi-54_Meals my-5 mr-10 d-iblock cls-meal-icon" />
                              {mealType.replace("_", " ")}
                            </Select.Option>
                          </Select>
                        </React.Fragment>
                    ...
                    >
                      <Text className="cls-seatlist_icon">
                        {value.icon ? (
                          ...
                          <Text
                            className={`cls-icon ${tabSelected === t("baggage") ? "Infi-57_BaggageLite" : "Infi-54_Meals"}`}
                          ></Text>
                        )}
            ...
            <Text className={`cls-grey f-reg d-block ${isSmallScreen ? "pt-5 fs-13" : "pt-10 fs-14"}`}>
              {t("selected")}{" "}
              <Text
              ...
              <Text
                className={`cls-dark-grey f-med pl-10 ${isSmallScreen ? "fs-13" : "fs-14"}`}
                style={{ textTransform: "lowercase" }}
              >
                {tabSelected === t("seat")
                  ? paxData?.filter(
                    ...
                    >
                      {
                        ((tabSelected === t("seat") &&
                          ...
                          (tabSelected === t("meals")
                            ...
                            : "")) && (
                          <Text
                            className="Infi-82_CloseMark fs-12 mr-5 cls-close-mark"
